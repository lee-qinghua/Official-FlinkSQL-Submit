...
select
    ...
    cast(if(sum(WHITE_LIST=1,1,0)=0,-999,sum (if(WHITE_LIST=1,CREDIT_SCORE_1,0))/sum(WHITE_LIST=1,1,0)) as double )     as      AVG_ACC_CCSCT,  -- 截止当前时点前贷款授信申请通过客户的客户卡评分均值
    cast(if(sum(WHITE_LIST=1,1,0)=0,-999,sum (if(WHITE_LIST=1,APPLY_SCORE,0))/sum(WHITE_LIST=1,1,0)) as double )        as      AVG_ACC_PCSCT   -- 截止当前时点前贷款授信申请通过客户的产品卡评分均值
...
select
    ...
    RATE_OC_2H,
    RATE_OC_6H
from mid_table2 b join no_window_source a on a.BUSINESS_TYPE_CD=b.BUSINESS_TYPE_CD and b.dt between a.et -interval '10' second and a.et;
insert into kafka_product_monitor_suixindai select * from final_sink;